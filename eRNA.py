...
def main(args):
    ...
    if not (demo and os.path.exists(fn_taginfo)):
        ...
        if status:
            ...
            return status
        if not status and not os.path.exists(fn_taginfo):
        ...
        if status:
            ...
            return status
        if not status and not os.path.exists(fn_peak_gtf):
            logger.error(f'Error: peak gtf file not generated by findPeaks: {fn_peak_gtf}\nstdout: {stdout}\nstderr: {stderr}')
            return 1
