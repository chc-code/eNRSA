...
There are 2 actions:
1. build: create the scripts for running NRSA for both old version NRASA and eNRSA, output = run_all_v1.lite.sh and run_all_v2.lite.sh
2. plot:  collect the performance data from the running log files and plot the results
...
def create_sh(pwd, config, n_rep, version=None, force=False):
    ...
    record = {}  # k1 = step1, step2, k2 = lb, v = [], each element is like [fn_sh, fn_log]
    v1_single_run_tests = {'16_sam', '8_sam', '17.8 GB', '4_sam', '6.8 GB', '3.6 GB', '1.4 GB', '2_sam'}
    v1_skip_tests = {'16_sam',}
    for c in config:
        lb = c['lb']
        ...
        if version == 'v1':
            if c['gtf']:
                continue
            if lb in v1_skip_tests:
                continue
        for step in ['step1', 'step2']:
            fn_nrsa_script = f'{pw_nrsa_code}/{executable_map[step]}'
            for i_rep in range(n_rep):
                i_rep += 1
                if version == 'v1' and lb in v1_single_run_tests and i_rep > 1:
                    break # only run 1 repeat for 8_sam
                sh = f'{pwd}/sh{pw_suffix}/{lb_in_fn}_{step}_{i_rep}.sh'
                log = f'{pwd}/sh{pw_suffix}/{lb_in_fn}_{step}_{i_rep}.log'

                record.setdefault(lb, {}).setdefault(i_rep, {})[step] = [sh, log]
                ...
                with open(sh, 'w') as f:
                    ...
                    f.write(f'echo "start {step} {lb} {i_rep}: " $(date)\n{rm_command}\n')
                    f.write(f'/usr/bin/time -v {pw_program} {fn_nrsa_script} {pwout_arg} {pwout} {organism} {arg_extra}')
                    ...
                    f.write(f'echo "end {step} {lb} {i_rep}: " $(date)\n')
    ...
    lb_order = sorted(record, key=lambda x: float(re.match(r'[\d\.]+', x).group()) * (1 if 'GB' in x else 100) if re.match(r'[\d\.]+', x) else 10000)
    
    for lb in lb_order:
        v1 = record[lb]
        lb_in_fn = re.sub(r'\s+', '_', lb)
        for i_rep, v2 in v1.items():
            cmd_tmp = []
            for step in ['step1', 'step2']:
                fn_sh, fn_log = v2[step]
                if not force and os.path.exists(fn_log):
                    ...
                    ct['new'] += 1
                    new_scripts.append(fn_sh)
                os.chmod(fn_sh, 0o755)
                cmd_tmp.append(fn_sh)
            if len(cmd_tmp) == 0:
                continue
            fn_wrap = f'{pwd}/sh{pw_suffix}/{lb_in_fn}_{i_rep}.wrap.sh'
            with open(fn_wrap, 'w') as f:
                print(' && '.join(cmd_tmp), file=f)
            os.chmod(fn_wrap, 0o755)

            sh_list_lite.append(fn_wrap)
                
    
    logger.info(f'version = {version} - {ct}')
    if new_scripts:
        logger.info(f'new scripts n = {len(new_scripts)}')
        print('\n'.join(new_scripts))
    
    if len(sh_list_lite) > 0:
        with open(f'{pwd}/run_all_{version}.lite.sh', 'w') as f:
            logger.info(f'sh_list_lite n = {len(sh_list_lite)}')
            tmp = '\n'.join(sh_list_lite)
            lineout = f'nohup parallel -j 4 ::: $(cat <<EOF\n{tmp}\nEOF\n) &> run_all_v1.lite.log & '
            print(lineout, file=f)
    elif os.path.exists(f'{pwd}/run_all_{version}.lite.sh'):
        os.unlink(f'{pwd}/run_all_{version}.lite.sh')
...
def plot_benchmark_results(df, fn_figure):
    ...
    n_row = 2
    n_col = 2
    gs = GridSpec(n_row * 2, n_col, height_ratios=[text_row_height, plot_row_height] * n_row, width_ratios=[0.95, 0.95],  figure=fig)
    ...
    # bar_colors = sns.color_palette()[:2][::-1]  # use the first 2 colors, reverse the order
    sns_default_colors = sns.color_palette('Set2')
    bar_colors = [sns_default_colors[i] for i in [2, 1]]  # use the first 2 colors, reverse the order
    ...
    for i, category in enumerate(categories):
        ...
        # Time plot
        # ax = axes[i, 0]
        for subplot_col_idx, df_y_col, ylabel, fmt in zip([0, 1], ['time_h', 'mem'], ['Time (h)', 'Memory (GB)'], ['%.2f', '%.2f']):
            ax = fig.add_subplot(gs[i * 2 + 1, subplot_col_idx])
            n_groups = len(data.version.unique())
            palette = bar_colors[-n_groups:]
            sns.barplot(x='lb', y=df_y_col, hue='version', data=data, 
                    capsize=0.1, errwidth=1, errorbar='sd', ax=ax, order=grouped.index, palette=palette)
            ...
            for container in ax.containers:
                labels = ax.bar_label(container, padding=bar_label_padding, fmt=fmt)
                plt.setp(labels, fontsize=tick_label_size - 1)
            ymin, ymax = ax.get_ylim()
            ax.set_ylim(ymin, ymax * 1.3)

            # legend
            legend = ax.legend(title=None, loc='upper left', fontsize=tick_label_size, frameon=False)
...
def collect_bench_res(script_settings):
    ...
    for version in ['v1', 'v2']:
        ...
        lb_order = sorted(config, key=lambda x: float(re.match(r'[\d\.]+', x).group()) * (1 if 'GB' in x else 100) if re.match(r'[\d\.]+', x) else 10000)
        for lb in lb_order:
            ...
            for i_rep, v2 in v1.items():
                for step in ['step1', 'step2']:
                    ...
                    if category == 'sam_number':
                        sam_number = int(lb.split("_")[0]) * 2 # change the sample number from number per condition to total number
                        lb_new = f'n={sam_number}'   
    ...
    df['version'] = df['version'].map({'v1': 'NRSA', 'v2': 'eNRSA'})
    for k in ['step1', 'step2', 'step1_step2']:
        df_k = df[df['step'] == k].copy()
